...
// Fixed By eslam ahmed   / To DIRECTORY_SEPARATOR
class My_Loader extends CI_Loader
{
    ...
    public function controller($controller, $name = '', $db_conn = FALSE,$params=array())
    {
        ...
        if (($last_slash = strrpos($controller, DIRECTORY_SEPARATOR)) !== FALSE)
        {
        ...
        {
            if ( ! file_exists($mod_path.'controllers'.DIRECTORY_SEPARATOR.$path.$controller.'.php'))
            {
                continue;
            }
            ...
            require_once($mod_path.'controllers'.DIRECTORY_SEPARATOR.$path.$controller.'.php');

            $controller = ucfirst($controller);
            if(count($params)>0){
                $refl = new ReflectionClass($controller);
                $CI->$name = $refl->newInstanceArgs($params);
            }else{
