(function(){
    ...
    $.fn.datagrid.defaults.filters = $.extend({}, $.fn.datagrid.defaults.editors, {
        label: {
            ...
            init: function(container, options){
                var input = $('<input type="text" dir="'+options.dir+'" class="datagrid-editable-input">').appendTo(container);
                return input;
            },
    ...
    function init(target, filters){
        ...
        var state = $.data(target, 'datagrid');
        var opts = state.options;
        var paging = $(target).datagrid('getPager');
        var the_menu = $('<div></div>').appendTo('body');
        the_menu.menu({
        ...
        var multi_sort_text = 'Multi sort';
        var single_sort_text = 'single sort';
        the_menu.menu('appendItem', {
            text: multi_sort_text,
            iconCls: '',
            onclick: function(){
                opts.multiSort = true;
                $(target).closest('div.panel').find('div.datagrid-sort-asc').removeClass('datagrid-sort-asc');
                $(target).closest('div.panel').find('div.datagrid-sort-desc').removeClass('datagrid-sort-desc');
                opts.sortName ='';
                opts.sortOrder ='asc';
                $(target).datagrid('reload');
                the_menu.find('.icon-ok').removeClass('icon-ok');
                var item_multi = the_menu.menu('findItem', multi_sort_text);
                the_menu.menu('setIcon', {
                    target: item_multi.target,
                    iconCls: 'icon-ok'
                });
            }
        });
        the_menu.menu('appendItem', {
            text: single_sort_text,
            iconCls: '',
            onclick: function(){
                opts.multiSort = false;
                $(target).closest('div.panel').find('div.datagrid-sort-asc').removeClass('datagrid-sort-asc');
                $(target).closest('div.panel').find('div.datagrid-sort-desc').removeClass('datagrid-sort-desc');
                opts.sortName ='';
                opts.sortOrder ='asc';
                $(target).datagrid('reload');
                the_menu.find('.icon-ok').removeClass('icon-ok');
                var item_single= the_menu.menu('findItem', single_sort_text);
                the_menu.menu('setIcon', {
                    target: item_single.target,
                    iconCls: 'icon-ok'
                });
            }
        });
        ...
        paging.pagination({
            buttons: [{
                ...
                iconCls:'my_sorting_list ico-share',
                handler:function(){
                    var my_ob = this;
                    the_menu.menu({
                        left: $(my_ob).offset().left,
                        top: $(my_ob).offset().top
                    });
                    the_menu.menu('show');
                }
            }]
        ...
        // $('span.my_sorting_list').replaceWith('<input id="sorting">');
        ...
        function createFilter(frozen){
            ...
            for(var i=0; i<fields.length; i++){
                ...
                if (field == '_'){continue;}
                var div = $('<div class="datagrid-filter-c align-center"></div>').appendTo(td);
        ...
        function createFilterButton(container, operators){
            ...
            btn.closest('div.datagrid-filter-c').find('input[type="text"]').keydown(function(e) {
                if(e.keyCode == 13) {
                    var oper = 'equal';
                    if($.inArray('contains',operators)>=0){
                        oper = 'contains';
                    }
